...
namespace CafebytesModels.HostedService
{
    public class DbSeederHostedService : IHostedService
    {
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            using (IServiceScope scope = serviceProvider.CreateScope())
            {
                ...
                var db = scope.ServiceProvider.GetRequiredService<CafebytesDbContext>();

                await SeedDbAsync(db);

            }
        }
        public async Task SeedDbAsync(CafebytesDbContext db)
        {
